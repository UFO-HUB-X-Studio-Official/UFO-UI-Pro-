--======================================================
-- UFO HUB X UI Pro | Core + External API (scroll L/R + icon/emoji)
--======================================================
...
local function stroke(gui,t,col,trans) local s=Instance.new("UIStroke",gui); s.Thickness=t or 1; s.Color=col or THEME.MINT; s.Transparency=trans or .3; s.ApplyStrokeMode=Enum.ApplyStrokeMode.Border; s.LineJoinMode=Enum.LineJoinMode.Round; return s end
...
--======================================================
-- 4) Build UI (ดีไซน์เดิม) + Scroll ซ้าย/ขวา
--======================================================
local function buildUI()
    ...
    glow.Size=UDim2.new(1.07,0,1.09,0); glow.Image=IMGS.GLOW; glow.ImageColor3=THEME.GREEN; glow.ImageTransparency=.78
    glow.ScaleType=Enum.ScaleType.Slice; glow.SliceCenter=Rect.new(24,24,276,276)

    local UIScale=Instance.new("UIScale",Window)
    local function fit()
        local v=(workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize) or Vector2.new(1280,720)
        UIScale.Scale=math.clamp(math.min(v.X/960,v.Y/600),0.8,1.05)
    end
    ...
    HeadAccent.Size=UDim2.new(1,-20,0,1); HeadAccent.BackgroundColor3=THEME.MINT; HeadAccent.BackgroundTransparency=.35

    local Dot=Instance.new("Frame",Header)
    Dot.BackgroundColor3=THEME.MINT; Dot.Position=UDim2.new(0,14,0.5,-4); Dot.Size=UDim2.new(0,8,0,8); Dot.BorderSizePixel=0; corner(Dot,4)
    ...
    Title.Text='<font color="#FFFFFF">UFO</font> <font color="#00FF8C">HUB X</font>'; Title.TextColor3=THEME.TEXT

    local BtnClose=Instance.new("TextButton",Header)
    BtnClose.Name="BtnClose"; BtnClose.Size=UDim2.new(0,24,0,24); BtnClose.Position=UDim2.new(1,-34,0.5,-12)
    BtnClose.BackgroundColor3=THEME.DANGER; BtnClose.Text="X"; BtnClose.Font=Enum.Font.GothamBold; BtnClose.TextSize=13; BtnClose.TextColor3=Color3.new(1,1,1)
    ...
    Inner.BackgroundColor3=THEME.BG_INNER; Inner.BorderSizePixel=0; Inner.Position=UDim2.new(0,8,0,8); Inner.Size=UDim2.new(1,-16,1,-16)
    corner(Inner,12)

    local Content=Instance.new("Frame",Body)
    Content.Name="Content"; Content.BackgroundColor3=THEME.BG_PANEL; Content.Position=UDim2.new(0,SIZE.GAP_OUTER,0,SIZE.GAP_OUTER)
    Content.Size=UDim2.new(1,-SIZE.GAP_OUTER*2,1,-SIZE.GAP_OUTER*2); corner(Content,12); stroke(Content,.8,THEME.MINT,.28)
    ...
    Left.Name="Left"; Left.BackgroundColor3=THEME.BG_INNER; Left.Size=UDim2.new(SIZE.LEFT_RATIO,-SIZE.GAP_BETWEEN/2,1,0)
    Left.ScrollBarThickness=3; Left.CanvasSize=UDim2.new(); Left.ClipsDescendants=true
    corner(Left,10); stroke(Left,1.1,THEME.GREEN,.08); stroke(Left,.6,THEME.MINT,.28)
    ...
    Right.Name="Right"; Right.BackgroundColor3=THEME.BG_INNER; Right.Position=UDim2.new(SIZE.LEFT_RATIO,SIZE.GAP_BETWEEN,0,0)
    Right.Size=UDim2.new(SIZE.RIGHT_RATIO,-SIZE.GAP_BETWEEN/2,1,0); Right.ScrollBarThickness=4; Right.ClipsDescendants=true; Right.CanvasSize=UDim2.new()
    corner(Right,10); stroke(Right,1.1,THEME.GREEN,.08); stroke(Right,.6,THEME.MINT,.28)
    ...
    do
        ...
        UFO.BackgroundTransparency=1; UFO.Image=IMGS.UFO; UFO.Size=UDim2.new(0,168,0,168); UFO.AnchorPoint=Vector2.new(.5,1)
        ...
        Halo.Size=UDim2.new(0,200,0,60); Halo.Image=IMGS.GLOW; Halo.ImageColor3=THEME.MINT_SOFT; Halo.ImageTransparency=.72; Halo.ZIndex=50
    end

    -- Toggle button (RightShift)
    do
        ...
        B.BackgroundColor3=Color3.new(0,0,0); B.BorderSizePixel=0; B.Image=IMGS.TOGGLE; corner(B,8); stroke(B,2,THEME.GREEN,0)
...
--======================================================
-- 5) UFOPro API (ภายนอกเรียกได้) — โฟกัสรองรับการสร้างปุ่ม/ไอเท็ม + icon/emoji
--======================================================
local UFOPro = { _version = "1.1-core" }
...
local function makeSideButton(parent, text, active)
    ...
    b.Name="Tab_"..text; b.AutoButtonColor=false; b.Size=UDim2.new(1,0,0,30)
    ...
    b.Parent=parent; corner(b,10); stroke(b,1,THEME.MINT,.22)
...
local function makeSection(parent, title)
    local holder=Instance.new("Frame",parent); holder.Size=UDim2.new(1,0,0,52); holder.BackgroundColor3=THEME.CARD_FILL; holder.BorderSizePixel=0
    corner(holder,10); stroke(holder,1,THEME.CARD_STROKE,.18)
    ...
    local lab=Instance.new("TextLabel",inner); lab.BackgroundTransparency=1; lab.Size=UDim2.new(1,-18,1,0); lab.Font=Enum.Font.GothamSemibold; lab.TextSize=16; lab.TextXAlignment=Enum.TextXAlignment.Left; lab.TextColor3=THEME.TEXT; lab.Text=title or "Section"
    ...
    -- API ของ section (รองรับไอคอนจาก asset id หรือ emoji)
    local api={}

    -- สร้างปุ่มทั่วไป พร้อมไอคอนภาพ/อิโมจิ (จากภายนอก)
    function api:NewButton(opts, callback)
        -- opts: {text="...", info="...", icon="rbxassetid://.. or '🔥'", height=30}
        opts = opts or {}
        local h = tonumber(opts.height) or 30
        local btn=Instance.new("TextButton",content)
        btn.AutoButtonColor=false; btn.Size=UDim2.new(1,0,0,h)
        btn.BackgroundColor3=THEME.BTN_IDLE; btn.Font=Enum.Font.GothamSemibold; btn.TextSize=14
        btn.TextXAlignment=Enum.TextXAlignment.Left; btn.TextColor3=THEME.TEXT
        btn.Text="   "..(opts.text or "Button"); corner(btn,8); stroke(btn,1,THEME.MINT,.2)
        ...
        if opts.icon then
            if isAssetId(opts.icon) then
                local img=Instance.new("ImageLabel",btn); img.BackgroundTransparency=1
                img.Size=UDim2.new(0,h-8,0,h-8); img.Position=UDim2.new(0,6,0,4)
                img.Image = opts.icon:match("^rbxassetid://") and opts.icon or ("rbxassetid://"..opts.icon)
                btn.Text = "      "..(opts.text or "Button")
            else
                local em=Instance.new("TextLabel",btn); em.BackgroundTransparency=1; em.Size=UDim2.new(0,h-6,1,0); em.Position=UDim2.new(0,6,0,0)
                em.Font=Enum.Font.GothamBold; em.TextScaled=true; em.TextColor3=THEME.TEXT; em.Text=tostring(opts.icon)
                btn.Text = "      "..(opts.text or "Button")
            end
        end
        return btn
    ...
    -- สร้าง “ไอเท็มทั่วไป” สำหรับต่อยอด (label/card/custom) + รองรับ icon เหมือนปุ่ม
    function api:NewItem(opts)
        -- opts: {height=30, text="...", icon=assetIdOrEmoji}
        opts = opts or {}
        local h = tonumber(opts.height) or 30
        local item = Instance.new("Frame",content)
        item.BackgroundColor3=THEME.BTN_IDLE; item.Size=UDim2.new(1,0,0,h); corner(item,8); stroke(item,1,THEME.MINT,.2)

        -- icon
        local leftPad = 6
        if opts.icon then
            if isAssetId(opts.icon) then
                local img=Instance.new("ImageLabel",item); img.BackgroundTransparency=1; img.Size=UDim2.new(0,h-8,0,h-8); img.Position=UDim2.new(0,6,0,4)
                img.Image=opts.icon:match("^rbxassetid://") and opts.icon or ("rbxassetid://"..opts.icon)
                leftPad = leftPad + (h-8) + 6
            else
                local em=Instance.new("TextLabel",item); em.BackgroundTransparency=1; em.Size=UDim2.new(0,h-6,1,0); em.Position=UDim2.new(0,6,0,0)
                em.Font=Enum.Font.GothamBold; em.TextScaled=true; em.TextColor3=THEME.TEXT; em.Text=tostring(opts.icon)
                leftPad = leftPad + (h-6) + 6
            end
        end

        if opts.text then
            local t=Instance.new("TextLabel",item); t.BackgroundTransparency=1; t.TextXAlignment=Enum.TextXAlignment.Left
            t.Position=UDim2.new(0,leftPad,0,0); t.Size=UDim2.new(1,-leftPad,1,0)
            t.Font=Enum.Font.Gotham; t.TextSize=14; t.TextColor3=THEME.TEXT; t.Text=tostring(opts.text)
        end
        return item
    end
    ...
    function api:NewLabel(text)
        ...
        l.TextColor3=THEME.TEXT; l.Text=text or "Label"
...
function UFOPro.CreateLib(title)
    ...
    local api = { _win=win, _left=left, _host=host, _tabs={}, _activeBtn=nil }
    ...
    function api:NewTab(name)
        ...
        -- ส่ง API ของแท็บ (สร้าง Section/ปุ่ม/ไอเท็มจาก "ภายนอก")
        local tabAPI = {}
...
-- local Sec = Tab:NewSection("Actions")
-- Sec:NewLabel("Status: Ready")
-- Sec:NewButton({text="Hello (emoji)", icon="👽"}, function() print("Hello!") end)
-- Sec:NewButton({text="With Roblox Image", icon="100650447103028"}, function() print("Clicked") end)
--===============================================================
